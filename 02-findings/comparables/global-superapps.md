...
**Scope caveat (read first):** All four are huge, scaled, capital-rich platforms in dense markets with a powerful distribution wedge (a marketplace, a social graph, telco reach, or QR ubiquity). Our DRC MVP is an early-stage **pass-through** *merchant-acquiring* app on rented rails (pawaPay): merchants onboard and are paid via a till/QR, the merchant absorbs the MDR, and a **first-class USSD channel** lets customers pay with no app. The transferable thing is the **order and the monetisation logic**, not the surface. The "What it means for us" section flags what does NOT transfer. *(Note 2026-06-11: this file originally described a consumer "pay any number" pass-through with no merchant onboarding; that framing is superseded - see the pivot notice in [`README.md`](README.md).)*
...
- **Merchant MDR is our rung 1, not a deferred rung 2 (updated post-pivot).** Because the MVP is merchant-acquiring, the merchant fee that GCash/Mercado Pago/Paytm reached *later* is our **primary revenue from day one**. The ladder's higher rungs (lending, wealth, float) stay deferred and, on a pure pass-through, are mostly gated behind licensing we don't hold. *(This bullet originally read "merchant MDR deferred / no merchant onboarding in v1" - reversed by the 2026-06-11 pivot.)* **[inference, Med]**
